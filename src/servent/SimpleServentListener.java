...
import servent.handler.snapshot.АVTellHandler;
		...
		while (working) {
			try {
				...
				synchronized (AppConfig.colorLock) {
					if (AppConfig.SNAPSHOT_TYPE == SnapshotType.AB) {
					...
					} else if (AppConfig.SNAPSHOT_TYPE == SnapshotType.AV) {
						if (clientMessage.isWhite() == false && AppConfig.isWhite.get()) {
							...
							if (clientMessage.getMessageType() != MessageType.LY_MARKER) {
								...
								AVBitcakeManager avBitcakeManager =
										(AVBitcakeManager)snapshotCollector.getBitcakeManager();
								avBitcakeManager.markerEvent(
