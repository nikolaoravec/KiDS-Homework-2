...
public class ABTellHandler implements MessageHandler {
	...
	private transient SnapshotCollector snapshotCollector;
	...
	public void run() {
		try {
			AppConfig.timestampedErrorPrint(clientMessage.getClass().getName());
			if (clientMessage.getMessageType() == MessageType.AB_TELL) {
				AppConfig.timestampedStandardPrint("class: " + clientMessage.getClass().toString());
